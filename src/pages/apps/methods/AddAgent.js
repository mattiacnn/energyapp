  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting }) => {
      try {
        ...
        const response = await axios.post('/pmethod/create', new_agent);
        const { newPMethod } = response.data;
        if (newPMethod) {
          // dispatch(openSnackbar('Customer Added Successfully'));
          dispatch(
            openSnackbar({
              open: true,
              message: 'Metodo di pagamento aggiunta con successo!',
              variant: 'alert',
              alert: {
                color: 'success'
              },
      ...
      <FormikProvider value={formik}>
        <LocalizationProvider dateAdapter={AdapterDateFns}>
          <Form autoComplete="off" noValidate onSubmit={handleSubmit}>
            <DialogTitle>{customer ? 'Modifica' : 'Nuovo metodo di pagamento'}</DialogTitle>
            <Divider />
