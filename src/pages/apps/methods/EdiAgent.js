  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting }) => {
      try {
        ...
        const response = await axios.put('/pmethod/update', new_agent);
        const { pmethod } = response.data;
        if (pmethod) {
        ...
        dispatch(
          openSnackbar({
            open: true,
            message: 'Errore durante l\'aggiunta del metodo di pagamento. Riprova più tardi.',
            variant: 'alert',
            alert: {
              color: 'error'
            },
                  ...
                  <Stack spacing={1.25}>
                    <ListItem divider>
                      <ListItemText
                        ...
                        primary="Disabilita metodo di pagamento"
                        secondary="Disabilitando il metodo di pagamento non verrà più visualizzato nel sistema"
                      />
                      <Switch
                        edge="end"
