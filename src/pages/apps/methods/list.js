...
const RatesListPage = () => {
  ...
  const fetchAgents = async () => {
    setLoading(true);
    const agents = await axios.get('/pmethod/list',{params: {hidden: showHidden}});
    ...
    try {
      ...
      if (deleteAgent.status === 200 && deleteAgent.data.deleted) {
        ...
        dispatch(
          openSnackbar({
            open: true,
            message: 'Non puoi cancellare questo metodo di pagamento perchè associato ad un contratto. Prova a disabilitarlo',
            variant: 'alert',
            alert: {
              color: 'error'
            },
