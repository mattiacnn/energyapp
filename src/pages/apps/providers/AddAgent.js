  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting }) => {
      try {
        ...
        const response = await axios.post('/provider/create', new_agent);
        ...
        if (newProvider) {
          ...
          dispatch(
            openSnackbar({
              open: true,
              message: 'Fornitore aggiunta con successo!',
              variant: 'alert',
              alert: {
                color: 'success'
              },
      ...
      <FormikProvider value={formik}>
        <LocalizationProvider dateAdapter={AdapterDateFns}>
          <Form autoComplete="off" noValidate onSubmit={handleSubmit}>
            <DialogTitle>{customer ? 'Modifica' : 'Nuovo Fornitore'}</DialogTitle>
            <Divider />
