...
const RatesListPage = () => {
  ...
  const fetchAgents = async () => {
    setLoading(true);
    const agents = await axios.get('/provider/list', { params: { hidden: showHidden } });
    setAgents(agents.data.providers);
  ...
  const handleDelete = async () => {
    try {
      const deleteAgent = await axios.delete(`/provider/${customerDeleteId.id}`);
      setOpen(false);
      dispatch(
        openSnackbar({
          open: true,
      ...
      if (deleteAgent.status === 200 && deleteAgent.data.deleted) {
        ...
        dispatch(
          openSnackbar({
            open: true,
            message: 'Non puoi cancellare questo fornitore perchè ha delle tariffe associate. Prova a disattivarlo',
            variant: 'alert',
            alert: {
              color: 'error'
            },
      ...
      dispatch(
        openSnackbar({
          open: true,
          message: 'Errore durante la cancellazione del Fornitore',
          variant: 'alert',
          alert: {
            color: 'error'
          },
