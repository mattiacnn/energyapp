  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting }) => {
      try {
        ...
        const response = await axios.post('/rate-type/create', new_agent);
        const { newRateType } = response.data;
        if (newRateType) {
          // dispatch(openSnackbar('Customer Added Successfully'));
          dispatch(
            openSnackbar({
              open: true,
              message: 'Tipo di tariffa aggiunta con successo!',
              variant: 'alert',
              alert: {
                color: 'success'
              },
        ...
        dispatch(
          openSnackbar({
            open: true,
            message: 'Errore durante l\'aggiunta del tipo di tariffa. Riprova più tardi.',
            variant: 'alert',
            alert: {
              color: 'error'
            },
