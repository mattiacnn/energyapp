...
const RatesListPage = () => {
  ...
  const fetchAgents = async () => {
    setLoading(true);
    const agents = await axios.get('/status/list',{params: {hidden: showHidden}});
    setAgents(agents.data.status);
  ...
  const handleDelete = async () => {
    try {
      const deleteAgent = await axios.delete(`/status/${customerDeleteId.id}`);
      setOpen(false);
      dispatch(
        openSnackbar({
          open: true,
          message: 'stato cancellato con successo!',
          variant: 'stato',
          alert: {
            color: 'success'
          },
      ...
      if (deleteAgent.status === 200 && deleteAgent.data.deleted) {
        ...
        dispatch(
          openSnackbar({
            open: true,
            message: 'Non puoi cancellare questo stato perchè associato ad un contratto. Prova a disabilitarlo',
            variant: 'alert',
            alert: {
              color: 'error'
            },
      ...
      dispatch(
        openSnackbar({
          open: true,
          message: 'Errore durante la cancellazione del stato',
          variant: 'alert',
          alert: {
            color: 'error'
          },
