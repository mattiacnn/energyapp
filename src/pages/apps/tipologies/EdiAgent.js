  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting }) => {
      try {
        ...
        const response = await axios.put('/tipology/update', new_agent);
        const { tipology } = response.data;
        if (tipology) {
          // dispatch(openSnackbar('Customer Added Successfully'));
          dispatch(
            openSnackbar({
              open: true,
              message: 'Tipologia modificata con successo!',
              variant: 'alert',
              alert: {
                color: 'success'
              },
        ...
        dispatch(
          openSnackbar({
            open: true,
            message: 'Errore durante l\'aggiunta della tipologia. Riprova più tardi.',
            variant: 'alert',
            alert: {
              color: 'error'
            },
                  ...
                  <Stack spacing={1.25}>
                    <ListItem divider>
                      <ListItemText
                        ...
                        primary="Disabilita tipologia"
                        secondary="Disabilitando la tipologia non verrà più visualizzata nel sistema"
                      />
                      <Switch
                        edge="end"
