...
const TabAttachments = () => {
  ...
  const fetchFiles = async () => {
    try {
      const response = await axios.get(`/client/files/${client.id}`);
  ...
  const downloadFile = async (file) => {
    try {
      ...
      form.action = `${apiUrl}/client/download/${client.id}/${file}`;
      form.method = 'GET';
  ...
  const handleConfirm = async () => {
    try {
      const res = await axios.delete(`/client/files/${client.id}/${selectedFile}`);
      if (res.status === 200) {
        dispatch(
          openSnackbar({
            open: true,
